    ...
    <div className={`relative flex flex-col justify-center lg:justify-between md:text-xs lg:text-sm items-center border-[1px] ${itemInCart ? "shadow-md" : "hover:shadow-md"} gap-3 py-5 md:px-2 group rounded-xl duration-150 text-center hover:cursor-pointer`}>
        ...
        <div className='w-full flex flex-col justify-center items-center gap-1 leading-4 px-4'>
          ...
          <div className={`flex gap-2 lg:gap-4 mt-2 w-full font-semibold transition-all duration-150 z-50 `}>
            ...
            <button className={`w-full flex items-center justify-center gap-2 px-4 py-2 text-[10px] rounded-lg ${itemInCart ? "bg-secondary-600 text-white" : "hover:bg-secondary-600 hover:text-[#fff] border-[1px] border-black"} duration-150`} onClick={()=>  toggleItem(item)}>
              <i className={`bi ${!itemInCart ? "bi-heart" : "bi-heart-fill"} text-lg`}></i> 
              <span>{itemInCart ? "In Wishlist" : "Wishlist"}</span>
            </button>

            <button className={`w-full flex items-center justify-center gap-2 px-2 md:px-4 py-2 text-[10px] rounded-lg ${itemInCart ? "bg-primary-600 text-white" : "bg-black/90 text-white hover:bg-primary-600 hover:text-[#fff] shadow-md"} duration-150`} onClick={()=>  toggleItem(item)}>
