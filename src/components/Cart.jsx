...
const Cart = ({ show, items, minus, itemNum, sum, handleShow }) => {

  return ( 
    <div className={`w-screen h-screen ${show ? "flex" : "hidden"} flex-col justify-center items-center bg-white/90 fixed z-50`}>
        <div className='w-[80%] h-[95%] bg-white shadow-lg rounded-lg flex flex-col justify-between items-center p-6'>
            ...
            <div className='w-full h-full grid grid-cols-5 gap-6'>
                <div className='col-span-3 w-full h-full flex flex-col gap-4 divide-y divide-[#342718]/10 overflow-y-scroll'>
                    {/* Cart items will be displayed here */}
                    {
                        items?.length > 0 ? 
                            items?.map((item) => {
                                return(
                                <Item key={item.id} item={item} handleMinus={minus} itemNum={itemNum} handleSum={sum}/>
                                )
                            }) :
                ...
                <div className='col-span-2 w-full h-fit p-4 border-[1px] border-[#342718]/10 rounded-lg'>
                    ...
                    <button className={`px-10 py-2 lg:py-4 w-full lg:mt-4 bg-[#2ECC71] hover:bg-[#27AE60] font-semibold text-white shadow-md duration-150`}>Checkout</button>
                </div>
            </div>
